...
#' Creates barplots, histograms
#' 
#' @name createBarplot
#' @usage createBarplot(table, column, path, na.rm, percent)
#' @description This function groups values by column and then creates a barplot with its
#' @description different values
#' @param table table from where it sources data
#' @param column which column to divide by
#' @param path unused
#' @param na.rm whether to count void values or not
#' @param percent show in percents or actual numbers
#' 
...
data <- function(name)
{
  ...
  {
    statement = sprintf("SELECT * FROM patient WHERE disease='%s'", name)
  }
  ...
  {
    #Tissue samples
    normal = "SELECT count(*) FROM normal"
    cancer = "SELECT count(*) FROM cancer"
  }
  else
  {
    #Tissue samples
    normal = sprintf("SELECT count(*) FROM normal WHERE disease='%s'", name)
    cancer = sprintf("SELECT count(*) FROM cancer WHERE disease='%s'", name)
  }
